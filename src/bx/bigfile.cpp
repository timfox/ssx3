// Retail cBigFile — native HAL implementation (decomp tree).
// PS2 matching: splat asm under src/bx/ when split; until then host path only.
...
unsigned long cBigFile_read(HostBigFile* self, const char* entry_name, void* dst, unsigned long dst_size) {
    ...
    if (!self->archive) {
        const auto bytes = host::read_disc_file(self->path[0] ? self->path : entry_name);
        if (!bytes || bytes->empty()) {
            return 0;
        }
        const unsigned long to_copy =
            bytes->size() < dst_size ? static_cast<unsigned long>(bytes->size()) : dst_size;
        std::memcpy(dst, bytes->data(), to_copy);
        return to_copy;
    }
